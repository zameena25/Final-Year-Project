...
from auth.app_paths import get_app_data_dir, get_logs_dir
...
_HONEYPOT_JSON = Path(__file__).parent / "logs" / "honeypots.json"
...
LOG_FILE        = get_logs_dir() / "novasphere.log"
QUARANTINE_DIR  = get_app_data_dir() / "quarantine"
MONITOR_DIR     = None
...
def icon_label(icon_name: str, color: str = "#e8eaf0", size: int = 20) -> QLabel:
    """Returns a QLabel showing a 2D vector icon instead of an emoji."""
    try:
        icon = qta.icon(icon_name, color=color)
    except Exception as e:
        print(f"[icon_label] Invalid icon name '{icon_name}': {e}")
        icon = qta.icon("fa5s.circle", color=color)  # safe fallback
...
def make_card(title: str = "", sub: str = "") -> tuple[QFrame, QVBoxLayout]:
    ...
    frame = QFrame()
    frame.setStyleSheet(
        f"QFrame{{background:{BG_CARD};border:1px;"
        f"border-radius:16px;}}"
    )
    lay = QVBoxLayout(frame)
    lay.setContentsMargins(38, 26, 38, 26)
    lay.setSpacing(10)
    if title:
        t = QLabel(title)
        t.setStyleSheet(
...
def try_import_detector() -> dict:
    """
    Import path_scores from ROOT detector.py - not the ransomware_part package copy.
    The package copy's path_scores is always empty since the app runs root detector.
    """
    try: 
        import sys, os 
...
def try_import_honeypots():
    """
    Read honeypot list from shared JSON written by HoneypotManager on setup.
    Never instantiates HoneypotManager - avoids re-creating bait files.
    Falls back to scanning monitored paths for known honeypot filenames.
    """
    if _HONEYPOT_JSON.exists():
        try:
            with open(_HONEYPOT_JSON, encoding="utf-8") as f:
                data = json.load(f)
            return [
                {
                    "name": Path(entry.get("path", "")).name, 
                    "path": entry.get("path", ""),
                    "intact": Path(entry.get("path", "")).exists(),
                }
                for entry in data
            ] 
        except Exception:
