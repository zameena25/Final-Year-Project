# frontend / live_monitoring_page.py
"""
...
def _load_recent_alert_findings(limit: int = 30) -> list[tuple[str, str, str, str]]:
    """Return persisted high-risk detections for the live-monitoring view."""
    ...
    try:
        ...
        try:
            return connection.execute(
                "SELECT file_path, severity, timestamp, alert_type FROM alerts "
...
def _score_row(filename: str, score: float) -> QFrame:
    ...
    name.setStyleSheet(
        f"color:{TEXT_WHITE};font-size:13px;"
    ...
    s_val.setStyleSheet(
        f"color:{score_col};font-size:13px;font-weight:700;"
...
def _log_row(line: str) -> QFrame:
    ...
    lbl = QLabel(line)
    lbl.setStyleSheet(
        f"color:{col};font-size:13px;font-family:Consolas,monospace;"
...
class LiveMonitoringPage(QWidget):
    ...
    def _build_ui(self):
        ...
        self._status_dot.setStyleSheet(
            f"color:{GREEN};font-size:15px;font-weight:600;background:transparent;"
        )
    ...
    def _refresh_data(self):
        ...
        alert_findings = _load_recent_alert_findings()

        # The behavioural detector keeps scores only in memory, while scans and
        # ransomware detections persist alerts. Merge both sources so every
        # detected threat is visible after navigation or an application restart.
        scores = dict(scores)
        alert_lines = []
        for file_path, severity, timestamp, alert_type in alert_findings:
            if not file_path:
                continue
            score = 100 if severity == "CRITICAL" else 85
            scores[file_path] = max(scores.get(file_path, 0), score)
            alert_lines.append(
                f"[{timestamp or '--'}] {severity or 'HIGH'} - "
                f"{alert_type or 'Threat detected'}: {file_path}"
            )
        lines = (lines + list(reversed(alert_lines)))[-30:]
