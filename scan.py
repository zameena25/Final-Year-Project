...
from .nova_style import (
    BG_MAIN, BG_CARD, BG_CARD2, BG_ROW, CYAN, CYAN_DIM, CYAN_GLOW, 
    BORDER, TEXT_WHITE, TEXT_MUTED, TEXT_SUB, BG_SIDEBAR, BG_TOPBAR, BG_MODULE, 
...
def stat_card(icon, value, label, special=None):
    ...
    if special == "dot":
        dl = QLabel("󰝥"); dl.setStyleSheet(f"color:{CYAN};font-size:10px;background:transparent;"); top.addWidget(dl)
        lay.addLayout(top)
...
class ScanWorker(QThread):
    ...
    SKIP_DIRS = {
        "Windows", "System32", "$Recycle.Bin", "__pycache__", 
        "node_modules", ".git", ".cache", "venv", ".venv", "site-packages",
    }
    ...
    def run(self):
        ...
        for root, dirs, files in os.walk(self._path, onerror=_on_walk_error):
            dirs[:] = [d for d in dirs if not d.startswith(".") and d not in self.SKIP_DIRS]
            for fname in files:
                all_files.append(os.path.join(root, fname))
                discovered += 1
                if discovered % 500 == 0:
                    self.progress.emit(0, max(discovered, 1))

        if skipped_dirs:
            print(f"[scan] WARNING: {len(skipped_dirs)} folders could not be accessed:")
            for err in skipped_dirs[:30]:
                print(f"  - {err}")

        total = len(all_files)
        print(f"[scan] Discovery complete: {total} files found")
        if total == 0:
            self.finished.emit(0, 0)
            return

        TARGET_SCAN_SECONDS = 3
        BATCH_SIZE = max(1,total // 100)
        delay_ms = max(1, min(15, int((TARGET_SCAN_SECONDS * 1000) / total)))
        ...
        for fpath in all_files:
            ...
            if is_threat:
                ...
                alert_record = {
                    ...
                    "alert_type": "SUSPICIOUS_FILE_DETECTED",
                    "severity": "HIGH",
                    "message": f"Suspicious file detected during manual scan: {fname}",
                    "file_path": fpath,
                    "source": "manual_scan",
                }
                try:
                    save_event(alert_record, is_alert=True)
                except Exception as e:
                    print(f"[scan] save_event failed for {fpath}: {e}")
        ...
        self.finished.emit(total, threats)
...
class ScanPage(QWidget):
    ...
    def __init__(self, parent=None):
        ...
        c1,self._fv = stat_card("󰂺","","Files Scanned")
        c2,self._tv = stat_card("󰯪","","Threats Detected")
        c3,_        = stat_card("󰕥","","Protection Status", special="active")
        c4, self._lv= stat_card("󱑃","","Last Scan Time",    special="time")
        ...
        mc1,_ = stat_card("󰂺","0","Files Scanned")
        mc2,_ = stat_card("󰯪","0","Threats Detected")
        mc3,_ = stat_card("󰕥","","Protection Status",special="active")
        mc4,_ = stat_card("󱑃","","Last Scan Time",   special="time")
    ...
    def _make_config(self):
        ...
        mods = [
            ("󰅟","File System Scan","Deep scan of all files for malware and ransomware signatures"),
            ("󱐋","Running Process Monitoring","Real-time analysis of active processes and memory behavior"),
            ("󰀓","Insider Behavior Analysis","Monitor user activity patterns for suspicious data exfiltration"),
            ("󰧮","Deception / Bait File Monitoring","Check honeypot files for unauthorized access attempts"),
            ("󰒓","Startup & Persistence Scan","Analyze registry keys, scheduled tasks, and autorun entries"),
            ("󰣺","External Device and Network Scan","Monitor USB devices and network connections for threats"),
        ]
    ...
    def _on_finished(self, total: int, threats: int):
        ...
        self.scan_complete.emit(total, threats)
