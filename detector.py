...
from .config import WINDOW_SECONDS, SETTINGS, LOG_FILE
from .prevention import suspend_process, take_action
...
def _rule_bulk_activity(path, now):
    ...
    recent_paths = set()
    for ts, etype, p in global_event_queue:
...
def _rule_deletion_spike(now):
    ...
    recent_deletes = sum(
        1 for ts, etype, p in global_event_queue
...
def _persist_alert(alert_type: str, severity: str, file_path: str, message: str):
    """Store ransomware detections in the alert data sources used by the UI."""
    alert = {
    ...
    try:
        ...
        # The detector must continue protecting files even if persistence fails.
        print(f"[ransomware] Failed to persist alert: {exc}")
...
def process_event(event, event_type: str, dest_path=None):
    ...
    if honeypot_mgr.is_honeypot(src_path):
        ...
        _persist_alert(
            ...
            "CRITICAL",
            src_path,
            f"Honeypot file accessed: {src_path}",
        )

        if cfg.SETTINGS.get("auto_kill_process", False):
            suspend_process(event)

        if cfg.SETTINGS.get("auto_quarantine", True):
            take_action(src_path, "CRITICAL", "HONEYPOT")
    ...
    global_event_queue.append((now, event_type, src_path)) # track deletes separately
...
def _respond_high(event, src_path, score):
    ...
    _persist_alert(
        ...
        "HIGH",
        src_path,
        f"High-risk ransomware activity detected (score {score:.0f}): {src_path}",
    )
    ...
    if cfg.SETTINGS.get("auto_quarantine", True):
        take_action(src_path, "HIGH")
    else:
        log_event("⚠️  Auto Quarantine OFF — manual review required")

def _get_recent_log_lines(n: int = 50) -> list:
    try:
...
def _detect_family(path_lower: str) -> str | None:
    families = {
        "ryuk":  [".ryk", ".ryuk"],
    ...
    for family, exts in families.items():
        if any(path_lower.endswith(e) for e in exts):
            return family
    return None
 
class RansomwareDetector:
    """
    Class wrapper around the module-level detection state.
    get_latest() returns the single highest-risk file as a flat dict
    so ransomware_part.py can call result.get("family") etc directly.
    """

    scores = path_scores #reference to module-level defaultdict

    def get_latest(self) -> dict:
        """
        Return the highest-scoring tracked file as a flat dict. 
        ...
        or {} if nothing is being tracked.
        """
        if not self.scores:
            return{}
        
        #picking the single highest-risk path
        raw_path = max(self.scores, key=lambda p: self.scores[p])
        score = self.scores[raw_path]
        path_lower = raw_path.lower()

        return {
            "file": raw_path.split("/")[-1].split("\\")[-1],
            "score": score,
            "family": _detect_family(path_lower),
            "extension": raw_path.rsplit(".", 1)[-1] if "." in raw_path else None,
            "ransom_note": any(p in path_lower for p in SUSPICIOUS_PATTERNS),
            "encryption": any(path_lower.endswith(e) for e in RANSOMWARE_EXTENSIONS),
        }
    
    # Helpers used by RansomwareDetector
    def _get_recent_log_lines(n: int = 50) -> list:
        try:
            with open(LOG_FILE, "r", encoding="utf-8") as f:
                return f.readlines()[-n:]
