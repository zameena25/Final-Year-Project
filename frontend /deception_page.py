...
def _honeypot_row(hp: dict) -> QFrame:
    ...
    icon = QLabel("󱃎" if intact else "󰀦")
    ...
    name.setStyleSheet(
        f"color:{TEXT_WHITE};font-size:15px;font-weight:600;"
...
class DeceptionPage(QWidget):
    ...
    def _build_ui(self):
        ...
        chart_labels = chart_frame.findChildren(QLabel)
        if len(chart_labels) >= 1:
            chart_labels[0].setStyleSheet(f"color:{TEXT_WHITE}; font-size:16px;font-weight:700;background:transparent;")
        if len(chart_labels) >= 2:
            chart_labels[1].setStyleSheet(f"color:{TEXT_MUTED};font-size:13px;background:transparent;")
        ...
        list_labels = list_frame.findChildren(QLabel)
        if len(list_labels) >= 1:
            list_labels[0].setStyleSheet(f"color:{TEXT_WHITE};font-size:16px;font-weight:700;background:transparent;padding-left:8px;")
        if len(list_labels) >= 2:
            list_labels[1].setStyleSheet(f"color:{TEXT_MUTED};font-size:13px;background:transparent;padding-left:8px;")
        list_lay.setSpacing(5)
