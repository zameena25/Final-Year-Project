...
DB_PATH     = _BASE /"logs" / "novasphere.db"
ALERT_JSONL = _BASE / "logs" / "alerts.jsonl"
...
def _card(title: str, sub: str = "") -> tuple:
    ...
    if title:
        t = QLabel(title)
        t.setStyleSheet(
            f"color:{TEXT_WHITE}; font-size:20px; font-weight:900; backound:transparent;"
        )
        lay.addWidget(t)
    if sub:
        s = QLabel(sub)
        s.setStyleSheet(f" color:{TEXT_MUTED}; font-size:15px; background:transparent;")
        lay.addWidget(s)
    return f, lay
...
def _table_header() -> QWidget:
    ...
    for col, width in [("USER", 180), ("ACTION", 180), ("TARGET", 140), ("RISK", 100)]:
        lbl = QLabel(col)
        lbl.setStyleSheet(
            f"color:{TEXT_MUTED};font-size:14px;font-weight:900;"
...
def _alert_to_row(a: dict) -> tuple:
    ...
    username   = a.get("username") or "System"
...
class TargetProfile(QFrame):
    def __init__(self, parent=None):
        ...
        hdr.setStyleSheet(
            f"color:{TEXT_MUTED}; font-size:15px; font-weight:900;"
...
class InsiderThreatPage(QWidget):
    ...
    def _build_ui(self):
        ...
        for key, icon, lbl, color in [
            ("total",    "🔔", "Total Alerts",  CYAN),
            ("critical", "🔴", "Critical",       RED),
            ("users",    "👤", "Users Flagged",  ORANGE),
            ("events",   "📄", "Events Logged",  GREEN),
        ]:
            f = QFrame()
            f.setStyleSheet(
                f"QFrame{{background:{BG_CARD};border:1px; border-radius:10px;}}"
            )
            f.setFixedHeight(76)
            ...
            ll = QLabel(lbl.upper())
            ll.setStyleSheet(
                f"color:{TEXT_MUTED};font-size:9px;letter-spacing:1px;background:transparent;"
            )
        ...
        risk_card, risk_lay = _card("⚠  High Risk Users", "Scored by severity × frequency")
        risk_card.setFixedWidth(320)
        ...
        act_card, act_lay = _card("🚨Recent Suspicious Activity")
        ...
        lc_card, lc_lay = _card("LOGIN ATTEMPTS (24H)")
    ...
    def _refresh(self):
        ...
        print(f"[DEBUG] Reading alerts from: {ALERT_JSONL.resolve()}")
        print(f"[DEBUG] File exists: {ALERT_JSONL.exists()}")
        print(f"[DEBUG] Alert count: {len(_load_alerts())}")
