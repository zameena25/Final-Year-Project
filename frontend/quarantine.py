...
from ransomware_part.config import QUARANTINE_FOLDER, BACKUP_FOLDER
...
class QuarantinePage(QWidget):
    ...
    def _build(self):
        ...
        title.setStyleSheet(
            f"color:{TEXT_WHITE};font-size:24px;font-weight:700;background:transparent;"
        )
        sub = QLabel("Isolated files blocked due to ransomware or suspicious behavior")
        sub.setStyleSheet(f"color:{TEXT_MUTED};font-size:14px;background:transparent;")
    ...
    # ── Actions ───────────────────────────────────────────────────────────────
    def _clear_all(self):
        """Permanently delete all files in quarantine folder."""
        if not os.path.isdir(QUARANTINE_FOLDER):
            return
    ...
    def _restore(self, entry):
        """Move file back from quarantine to backup folder (safe restore location)."""
        src = entry["quarantine_path"]
        if not os.path.exists(src):
            self.reload()
            return
        os.makedirs(BACKUP_FOLDER, exist_ok=True)
        dest = os.path.join(BACKUP_FOLDER, f"restored_{entry['display_name']}")
        try:
            import shutil
