...
from PyQt6.QtWidgets import (
    ...
    QPushButton, QScrollArea, QSizePolicy, QCheckBox,QStackedWidget, QDialog, QTextEdit
)
...
from .insider_threat import _load_events
from .scan import ScanPage

# ── Colors ────────────────────────────────────────────────────────────────────
from .nova_style import (
    BG_MAIN, BG_CARD, BG_CARD2, BG_ROW, CYAN, CYAN_DIM,
    BORDER, TEXT_WHITE, TEXT_MUTED, TEXT_SUB, BG_STATUS,
    RED, ORANGE, GREEN, YELLOW, BLUE, icon_label
)
...
def _count_alerts():
    """Get the TRUE total alert count, unbounded by any LIMIT."""
    c = _conn()
    if c:
        try:
            cur = c.cursor()
            ...
            cur.execute("SELECT COUNT(*) FROM alerts WHERE alert_type LIKE '%INSIDER%' OR alert_type LIKE '%RAPID%'")
            insider = cur.fetchone()[0]
            c.close()
            return total, active, insider
        except Exception:
            pass
    return None
...
def _stat_card(icon_name, label, value, sub="", color=CYAN, bg=BG_CARD):
    f = QFrame()
    ...
    lbl.setStyleSheet(f"color:{TEXT_MUTED};font-size:10px;letter-spacing:1px;background:transparent;")
    ic = icon_label(icon_name, color=color, size=20)
    ic.setStyleSheet(f"color:{color};font-size:20px;background:transparent;")
    top.addWidget(lbl); top.addStretch(); top.addWidget(ic)
    lay.addLayout(top)

    vl = QLabel(str(value))
    vl.setStyleSheet(f"color:{TEXT_WHITE};font-size:28px;font-weight:800;background:transparent;")
    lay.addWidget(vl)

    if sub:
        sl = QLabel(sub)
        sl.setStyleSheet(f"color:{color};font-size:12px;background:transparent;")
        lay.addWidget(sl)
...
def _incident_row(inc_id, inc_type, source, severity, status, time_ago) -> QWidget:
    ...
    def badge(text, color):
        ...
        b.setStyleSheet(
            f"color:{color};border:1px;border-radius:5px;"
            f"font-size:11px;font-weight:700;padding:2px 6px;background:transparent;"
        )
        return b
...
class LiveFileActivity(QFrame):
    def __init__(self, parent=None):
        ...
        hdr = QHBoxLayout()
        title = QLabel("󰉁  Live File Activity")
        title.setStyleSheet(f"color:{TEXT_WHITE};font-size:15px;font-weight:700;background:transparent;")
        dot = QLabel("󰝦 Real-time")
...
class SecurityOverviewPage(QWidget):
    ...
    def _live_tick(self):
        ...
        events_path = get_logs_dir() / "events.jsonl"
        modified = renamed = encrypted = 0
                    ...
                    for line in f:
                        ...
                        try:
                            ...
                            if etype == "MODIFIED":
                                ...
                                renamed += 1
                            if e.get("file_extension", "") in (".locked", ".encrypted", ".enc"):
                                encrypted += 1
                        except Exception:
                            continue
            except Exception:
                pass
        self._live_activity.update_stats(modified, renamed, encrypted)
    ...
    def _run_startup_scan(self):
        """Called once when the app launches: show the scan page and run automatically."""
    ...
    def _show_detailed_report(self):
        alerts = _load_alerts()
        dialog = QDialog(self)
        dialog.setWindowTitle("Detailed Security Report")
        dialog.resize(700, 500)
        lay = QVBoxLayout(dialog)

        header = QLabel(f"Full Scan Report — {len(alerts)} recorded alert(s)")
        header.setStyleSheet(f"color:{TEXT_WHITE};font-size:16px;font-weight:700;")
        lay.addWidget(header)

        text = QTextEdit()
        text.setReadOnly(True)
        text.setStyleSheet(f"background:{BG_CARD2};color:{TEXT_WHITE};font-family:Consolas,monospace;font-size:12px;")
        lines = []
        for a in alerts[:200]:
            lines.append(
                f"[{a.get('timestamp','—')}] {a.get('severity','—'):8} "
                f"{a.get('alert_type','—'):30} {a.get('file_path','')}"
            )
        text.setPlainText("\n".join(lines) if lines else "No alerts recorded yet.")
        lay.addWidget(text)
    ...
    def _build_ui(self):
        ...
        rt_lbl.setStyleSheet(f"color:{TEXT_WHITE};font-size:13px;background:transparent;")
        ...
        sb_icon = QLabel("󰒘")
        sb_icon.setStyleSheet(f"color:{GREEN};font-size:16px;background:transparent;")
        self._status_lbl = QLabel("System Status:  ")
        self._status_lbl.setStyleSheet(f"color:{TEXT_WHITE};font-size:15px;background:transparent;")
        ...
        self._status_dot = QLabel("󰝦")
        self._status_dot.setStyleSheet(f"color:{GREEN};font-size:16px;background:transparent;")
        ...
        c1, v1 = _stat_card("fa5s.shield-alt", "Total Threats", "0", "+0%", RED, BG_CARD)
        c2, v2 = _stat_card("fa5s.exclamation-triangle", "Active Alerts", "0", "+0", ORANGE, BG_CARD)
        c3, v3 = _stat_card("fa5s.bolt", "System Health", "98%", "Stable", GREEN, BG_CARD)
        c4, v4 = _stat_card("fa5s.user-secret", "Insider Risks", "0", "+0", YELLOW, "#1a1608")
        ...
        donut_card.setStyleSheet(f"QFrame{{background:{BG_CARD};border:1px;border-radius:14px;}}")
        ...
        ts_lbl.setStyleSheet(f"color:{TEXT_MUTED};font-size:10px;letter-spacing:1px;background:transparent;")
        ...
        chart_card = QFrame()
        chart_card.setStyleSheet(f"QFrame{{background:{BG_CARD};border:1px;border-radius:14px;}}")
        ...
        ch_title.setStyleSheet(f"color:{TEXT_WHITE};font-size:15px;font-weight:700;background:transparent;")
        ...
        risk_card = QFrame()
        risk_card.setStyleSheet(f"QFrame{{background:{BG_CARD};border:1px;border-radius:14px;}}")
        ...
        r_title.setStyleSheet(f"color:{TEXT_WHITE};font-size:15px;font-weight:700;background:transparent;")
        ...
        det.setStyleSheet(f"color:{CYAN};font-size:13px;background:transparent;")
        ...
        sc_hdr = QHBoxLayout()
        sc_icon = QLabel("󰄬")
        sc_icon.setStyleSheet("font-size:14px;background:transparent;")
        sc_title = QLabel("Last Scan Info")
        sc_title.setStyleSheet(f"color:{TEXT_WHITE};font-size:15px;font-weight:700;background:transparent;")
        sc_hdr.addWidget(sc_icon); sc_hdr.addWidget(sc_title); sc_hdr.addStretch()
        sc_lay.addLayout(sc_hdr)

        self._scan_time_lbl   = QLabel("󰅐  Last Scan:  Never")
        self._scan_type_lbl   = QLabel("󱎶  Scan Type:  —")
        self._scan_result_lbl = QLabel("󰄬  Result:  No scan run yet")
        ...
        rpt_btn.clicked.connect(self._show_detailed_report)
        sc_lay.addStretch()
        ...
        inc_card = QFrame()
        inc_card.setStyleSheet(f"QFrame{{background:{BG_CARD};border:1px;border-radius:14px;}}")
        ...
        inc_title.setStyleSheet(f"color:{TEXT_WHITE};font-size:15px;font-weight:700;background:transparent;")
        view_all = QPushButton("View All")
        view_all.setStyleSheet(
            f"QPushButton{{background:transparent;border:none;color:{CYAN};font-size:14px;}}"
        ...
        for col, w in [("INCIDENT ID", 0), ("TYPE", 150), ("SOURCE", 150), ("SEVERITY", 150), ("STATUS", 150), ("TIME", 150)]:
            lbl = QLabel(col)
        ...
        self._scan_page._worker = None
        self._scan_page.scan_complete.connect(self._on_scan_done)
        self._stack.addWidget(self._scan_page)

        back_btn = QPushButton("󰁍  Back to Overview")
    ...
    def _on_scan_done(self, total: int, threats: int):
        print(f"[dashboard] _on_scan_done called: total={total} threats={threats}")
        result_str = f"{threats} threat{'s' if threats != 1 else ''} found"
        color = "#ff4757" if threats > 0 else "#26de81"
        self.notify_scan_complete(
    ...
    def _refresh(self):
        alerts = _load_alerts()
        events = _load_events()
        print(f"[dashboard] _refresh called: {len(alerts)} alerts loaded from DB/jsonl")

        counts = _count_alerts()
        print(f"[dashboard] _count_alerts returned: {counts}")
        if counts:
            total, active, insider = counts
        else:
            total  = len(alerts)
        ...
        # The lifetime total grows forever, so use a five-minute active
        # monitoring window for the live health indicator.
        active_cutoff = datetime.now() - timedelta(minutes=5)
        recent_active = 0
        for alert in alerts:
            if (alert.get("severity") or "").upper() not in ("CRITICAL", "HIGH"):
                continue
            try:
                when = datetime.strptime(alert["timestamp"], "%Y-%m-%d %H:%M:%S")
                recent_active += when >= active_cutoff
            except (KeyError, TypeError, ValueError):
                continue

        threat_pct = min(recent_active * 20, 100)
        health_pct = 100 - threat_pct

        self._stat_vals["threats"].setText(str(total))
        self._stat_vals["alerts"].setText(str(recent_active))
        self._stat_vals["insider"].setText(str(insider))
        self._stat_vals["health"].setText(f"{health_pct}%")
        ...
        if scores:
            ...
            empty = QLabel("No user activity recorded")
            ...
            self._risk_container.addWidget(empty)
