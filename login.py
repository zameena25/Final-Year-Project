...
from .nova_style import icon_label
...
class IconLineEdit(QWidget):
    ...
    def __init__(self, placeholder: str, icon: str, is_password=False, parent=None):
        ...
        # Left icon
        icon_lbl = icon_label(icon, color=TEXT_MUTED, size=20)
        ...
        if is_password:
            ...
            self._eye_btn.setIcon(qta.icon("mdi.eye", color=TEXT_MUTED))
...
class LoginForm(QWidget):
    ...
    def __init__(self, parent=None):
        ...
        self._email = IconLineEdit("Enter your email", "mdi.email")
        ...
        self._password = IconLineEdit("Enter your password", "mdi.lock", is_password=True)
        ...
        self._error_lbl.setStyleSheet(
            f"color: #ff4757; font-size: 12px; background: transparent; padding: 4px 0;"
        )
        ...
        # Notice boxes
        lay.addWidget(NoticeBox("󰀨", "Authorized Access Only",
            "This system is restricted to authorized users. "
            "All activities are monitored and logged.",
            icon_color="#ffa726"))

        lay.addWidget(NoticeBox("󰦯", "",
    ...
    def _do_login(self):
        ...
        if not result["success"]:
            self._error_lbl.setText(result.get("error", "Invalid username or password."))
            self._error_lbl.setVisible(True)
            return
        user = result["user"]
        if result["requires_2fa"]:
            dialog = TwoFactorDialog(user["id"], parent=self)
            if dialog.exec() != TwoFactorDialog.DialogCode.Accepted or not dialog.verified:
                self._error_lbl.setText("Two-factor verification failed.")
                self._error_lbl.setVisible(True)
                return
        ...
        self.login_success.emit(user.get("full_name") or email)
...
class SignUpForm(QWidget):
    ...
    def __init__(self, parent=None):
        ...
        name_lbl.setStyleSheet(f"color: {TEXT_WHITE}; font-size: 15px; font-weight: 600; background: transparent;")
        self._name = IconLineEdit("Enter your full name", "mdi.account-circle")
        ...
        pass_lbl = QLabel("Password")
        pass_lbl.setStyleSheet(f"color: {TEXT_WHITE}; font-size: 15px; font-weight: 600; background: transparent;")
        self._password = IconLineEdit("Create a password", "mdi.lock", is_password=True)
        lay.addWidget(pass_lbl)
        lay.addWidget(self._password)
        ...
        self._confirm = IconLineEdit("Confirm your password", "mdi.lock", is_password=True)
        ...
        # Notice boxes
        lay.addWidget(NoticeBox("󰵂", "Email Notifications",
    ...
    def _do_signup(self):
        ...
        if not full_name or not email or not password:
            ...
            return
        if password != confirm:
            self._error_lbl.setText("Passowrds do not match.")
            self._error_lbl.setVisible(True)
            return
        result = self.auth_svc.register(email, password, full_name)
        if result["success"]:
            self.signup_success.emit()
        else:
            self._error_lbl.setText(result.get("error", "Sign up failed. Try again."))
            self._error_lbl.setVisible(True)
...
class AuthScreen(QWidget):
    ...
    def _on_guest(self):
        ...
        result = AuthService().guest_access(machine_id)
        if result["success"]:
            uses_left = result.get("uses_left", 0)
            print(f"Guest access granted. {uses_left} uses remaining.")
...
class MainWindow(QMainWindow):
    def __init__(self):
        ...
        self._dashboard_launched = False
        self._show_auth()
    ...
    def _show_auth(self):
        ...
        if token:
            ...
            if user_info:
                self._on_login(user_info.get("full_name") or user_info["username"])
                return
    ...
    def _on_login(self, user: str):
        ...
        try:
            from .main_window import NovaSphereWindow
            self.hide()
            self._dashboard = NovaSphereWindow(current_user=user)
            self._dashboard_launched = True
            ...
            print(err_text)
            self._dashboard_launched = False
            # Show the real error instead of a silent placeholder
            placeholder = QWidget()
            lay = QVBoxLayout(placeholder)
            title = QLabel(f"Logged in as: {user}\n\nDashboard failed to load:")
            title.setAlignment(Qt.AlignmentFlag.AlignCenter)
            title.setStyleSheet("font-weight: bold;")
            detail = QLabel(err_text)
            detail.setWordWrap(True)
            detail.setStyleSheet("color: #ff6b6b; font-size: 11px; font-family: Consolas, monospace;")
            scroll = QScrollArea()
            scroll.setWidgetResizable(True)
...
def main():
    ...
    print("STEP 1: splash shown")
    init_db()
    print("STEP 2: init_db done")
    ...
    print("STEP 3: palette set")

    window = MainWindow()
    print("STEP 4: MainWindow created")
    if not window._dashboard_launched:
        window.show()
    else:
        window._dashboard.show()

    splash.finish(window if not window._dashboard_launched else window._dashboard)
    sys.exit(app.exec())
